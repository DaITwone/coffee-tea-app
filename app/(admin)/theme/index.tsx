...
export default function ThemeManager() {
    ...
    const getPublicImageUrl = (path?: string | null, bucket = "products") => {
        if (!path) return null;

        const clean = String(path).trim().replace(/^\/+/, "");

        // Nếu DB lưu full URL thì dùng luôn
        ...
        // Cache bust để tránh RN cache file cũ / đổi ảnh không thấy
            ...
            <ScrollView className="px-5 pt-6" showsVerticalScrollIndicator={false}>
                    ...
                    {themes.map((item) => {
                        ...
                        return (
                            <Pressable
                                ...
                                className={`mr-4 rounded-2xl overflow-hidden border-2 ${item.is_active ? "border-green-500" : "border-gray-200"
                                    }`}
                            >
                                {bgUrl && (
                                    <Image
                                ...
                                <View className="bg-white">
                                    {/* Top divider */}
                                    <View
                                        className={`h-px ${item.is_active ? "bg-green-500 h-0.5" : "bg-gray-200"
                                            }`}
                                    />
                ...
                <View className="flex-row mb-4">
                    <Pressable
                        ...
                        className={`mr-3 px-4 py-2 rounded-full border ${bannerTheme === "standard"
                            ? "bg-green-600 border-green-500"
                            : "border-gray-300"
                            }`}
                    >
                        <Text
                            className={`text-sm font-semibold ${bannerTheme === "standard"
                                ? "text-white"
                                : "text-gray-700"
                                }`}
                        >
                    ...
                    <Pressable
                        ...
                        className={`px-4 py-2 rounded-full border ${bannerTheme === "spring"
                            ? "bg-green-600 border-green-500"
                            : "border-gray-300"
                            }`}
                    >
                        <Text
                            className={`text-sm font-semibold ${bannerTheme === "spring"
                                ? "text-white"
                                : "text-gray-700"
                                }`}
                        >
                ...
                <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                    {bannerPreview.map((item) => {
                        const imageUrl = getPublicImageUrl(item.image, "banners");
